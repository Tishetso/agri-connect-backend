...
public class NotificationService{
    ...
    public void notifyFarmerNewOrder(Order order){
        ...
        body.append("<p style='margin-top: 30px; color: #7f8c8d; font-size: 12 px;'>This is an automated message from AgriConnect. Please do not reply to this email.</p>");
        body.append("</div></div></body></html>");

        sendEmail(farmerEmail, subject, body.toString());
        sendSMS(order.getFarmer().getPhone(), "New order #" + order.getId() + " from " + consumerName + ". Total: R" + String.format("%.2f", order.getGrandTotal()));
    ...
    public void notifyConsumerOrderPlaced(Order order){
        ...
        body.append("<html><body> style='Font-family: Arial, sans-serif;'>");
        body.append("<div style='max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f8f9fa;'>");
        body.append("<div style='background-color: white; padding: 30px; border-radius: 10;'>");
